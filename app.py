...
def extrair_pdf(arquivo_bytes):
    ...
    for m in re.finditer(
        ...
        data = f"2026-{data_raw[3:5]}-{data_raw[0:2]}"
...
def salvar(cur, transacoes):
    ...
    for t in transacoes:
        try:
            cur.execute(
                "INSERT INTO transacoes VALUES (%s,%s,%s,%s,%s,NULL) ON CONFLICT DO NOTHING",
...
@app.route("/api/transacoes/categoria", methods=["POST"])
def atribuir_categoria():
    d = request.json
    tid = d.get("id")
    categoria = d.get("categoria")
    if not tid or not categoria:
        return jsonify({"erro": "id e categoria obrigatorios"}), 400
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
...
if __name__ == "__main__":
    app.run(debug=True)    cur.execute("""
        CREATE TABLE IF NOT EXISTS categorias (
            id SERIAL PRIMARY KEY,
            nome TEXT UNIQUE NOT NULL,
            cor TEXT DEFAULT '#388bfd',
